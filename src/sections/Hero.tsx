...
const Hero = () => {
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 relative z-20">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          {/* Profile image with 3D effects (FIRST on mobile, SECOND on desktop) */}
          <div
            ref={imageRef}
            className="order-1 lg:order-2 relative flex justify-center lg:justify-end mb-4 lg:mb-0"
            style={{ transformStyle: 'preserve-3d' }}
          >
            <div className="relative group">
              {/* Outer glow rings */}
              <div
                className="absolute inset-0 rounded-full border-2 border-primary/30 animate-[spin_10s_linear_infinite]"
                style={{ transform: 'scale(1.2)' }}
              />
              <div
                className="absolute inset-0 rounded-full border border-cyan-500/20 animate-[spin_15s_linear_infinite_reverse]"
              ...
              <div
                className="relative w-56 h-56 sm:w-80 sm:h-80 lg:w-96 lg:h-96 rounded-full overflow-hidden border-4 border-primary/30 p-2 sm:p-3 lg:p-4 animate-float"
                style={{
                  boxShadow:
                    '0 0 60px hsl(199, 89%, 48%, 0.3), inset 0 0 60px hsl(199, 89%, 48%, 0.1)',
                }}
              >
                <img
                ...
                <div
                  ...
                  style={{
                    background:
                      'linear-gradient(135deg, transparent 40%, rgba(56, 189, 248, 0.4) 50%, transparent 60%)',
              ...
              <div
                className="hidden sm:block absolute -top-4 -right-4 glass rounded-full px-4 py-2 animate-float border border-primary/30"
                style={{ animationDelay: '1s' }}
              >
                <span className="text-sm font-bold text-primary font-mono">GPA: 3.9/4.0</span>
              </div>
              <div
                className="hidden sm:block absolute -bottom-4 -left-4 glass rounded-full px-4 py-2 animate-float border border-primary/30"
                style={{ animationDelay: '2s' }}
              >
                <span className="text-sm font-bold text-primary font-mono">Dean's Honour List</span>
              </div>

              {/* Tech stack floating around (hide on mobile to prevent overflow) */}
              <div
                className="hidden sm:block absolute top-1/3 -left-8 glass rounded-lg px-2 py-1 text-xs text-emerald-400 ring-1 ring-emerald-500/30 animate-float"
                style={{ animationDelay: '0.5s', boxShadow: '0 0 16px rgba(16,185,129,0.25)' }}
              >
                React
              </div>
              <div
                className="hidden sm:block absolute top-1/3 -right-8 glass rounded-lg px-2 py-1 text-xs text-blue-400 ring-1 ring-blue-500/30 animate-float"
                style={{ animationDelay: '1.5s', boxShadow: '0 0 16px rgba(59,130,246,0.25)' }}
              >
                Python
              </div>
              <div
                className="hidden sm:block absolute bottom-1/3 -left-12 glass rounded-lg px-2 py-1 text-xs text-emerald-400 ring-1 ring-emerald-500/30 animate-float"
                style={{ animationDelay: '2.5s', boxShadow: '0 0 16px rgba(16,185,129,0.25)' }}
              >
                Node.js
              </div>
            </div>
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-4 justify-center lg:justify-start">
              ...
              <span className="text-primary text-sm font-mono">
                ...
                <span className="text-cyan-400">~</span> ./introduce.sh
              </span>
            </div>
